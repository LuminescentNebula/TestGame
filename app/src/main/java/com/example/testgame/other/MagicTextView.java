...
public class MagicTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    public void init(AttributeSet attrs){
        ...
        if(attrs != null){
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.magic_text_view);

            String typefaceName = a.getString( R.styleable.magic_text_view_typeface);
            if(typefaceName != null) {
                Typeface tf = Typeface.createFromAsset(getContext().getAssets(), String.format("fonts/%s.ttf", typefaceName));
                setTypeface(tf);
            }

            if(a.hasValue(R.styleable.magic_text_view_mtv_foreground)){
                Drawable foreground = a.getDrawable(R.styleable.magic_text_view_mtv_foreground);
                if(foreground != null){
                    this.setForegroundDrawable(foreground);
                }else{
                    this.setTextColor(a.getColor(R.styleable.magic_text_view_mtv_foreground, 0xff000000));
                }
            }

            if(a.hasValue(R.styleable.magic_text_view_mtv_background)){
                Drawable background = a.getDrawable(R.styleable.magic_text_view_mtv_background);
                if(background != null){
                    this.setBackgroundDrawable(background);
                }else{
                    this.setBackgroundColor(a.getColor(R.styleable.magic_text_view_mtv_background, 0xff000000));
                }
            }

            if(a.hasValue(R.styleable.magic_text_view_inner_shadow_color)){
                this.addInnerShadow(a.getDimensionPixelSize(R.styleable.magic_text_view_inner_shadow_radius, 0),
                        a.getDimensionPixelOffset(R.styleable.magic_text_view_inner_shadow_dx, 0),
                        a.getDimensionPixelOffset(R.styleable.magic_text_view_inner_shadow_dy, 0),
                        a.getColor(R.styleable.magic_text_view_inner_shadow_color, 0xff000000));
            }

            if(a.hasValue(R.styleable.magic_text_view_outer_shadow_color)){
                this.addOuterShadow(a.getDimensionPixelSize(R.styleable.magic_text_view_outer_shadow_radius, 0),
                        a.getDimensionPixelOffset(R.styleable.magic_text_view_outer_shadow_dx, 0),
                        a.getDimensionPixelOffset(R.styleable.magic_text_view_outer_shadow_dy, 0),
                        a.getColor(R.styleable.magic_text_view_outer_shadow_color, 0xff000000));
            }

            if(a.hasValue(R.styleable.magic_text_view_stroke_color)){
                float strokeWidth = a.getDimensionPixelSize(R.styleable.magic_text_view_stroke_width, 1);
                int strokeColor = a.getColor(R.styleable.magic_text_view_stroke_color, 0xff000000);
                float strokeMiter = a.getDimensionPixelSize(R.styleable.magic_text_view_stroke_miter, 10);
                Join strokeJoin = null;
                switch(a.getInt(R.styleable.magic_text_view_stroke_join_style, 0)){
